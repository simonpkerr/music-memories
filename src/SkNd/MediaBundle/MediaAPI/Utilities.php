...
class Utilities {
    ...
    public static function formatSearchString(array $params){
        ...
        $keywordQuery =  trim(strtolower(preg_replace('/(\w*)(\d*)(\s{0}(the complete|series|box set|complete|bbc|dvd)\s{0}|(\s??\[(dvd|blu\-ray)\]\s??)|(\d{1}(\s??|\-??)disc set)|(\[)|(\])|(\s??\:\s+?)|(\s??(\&)\s??)|(\s+?\d{1}\s+?))/i','$1$2', $keywords)));
